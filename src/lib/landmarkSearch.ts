...
// A search across the chosen city and the towns next to it: every match
// inside the city first, then the neighbours', each group still in the order
// searchLandmarks ranked it. A weaker match in town beats a better one a town
// over — the city was picked for a reason. A town further away is not
// searched at all: its places need the City picker changed first.
