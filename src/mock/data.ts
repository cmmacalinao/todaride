...
// sweep: amenity/shop/office/tourism/leisure/craft/healthcare, deduped across
// both cities' bounding boxes since San Jose City and Muñoz are adjacent and
// the padded boxes overlap). Category is inferred from the OSM tag — most
// don't map onto this app's small category set, so they land under 'other'
// rather than being mis-slotted. Excludes OSM's own noise for this area:
// office=telecommunication (cell towers, not destinations) and
// shop=variety_store (every one of ~55 is tagged with the proprietor's own
// name instead of a store name — not something a rider would search for) are
// dropped wholesale, and a few individually-junk names (bare "r", phone
// numbers standing in for a name, a "... Anthony"/"... MBA" surveyor-artifact
// suffix seen repeated across several categories) are filtered by name.
