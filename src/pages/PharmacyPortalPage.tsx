...
export function PharmacyPortalPage() {
  ...
  if (!loggedInPharmacyId) {
    ...
    // Reachable after all: a bookmark, a shared link, or a session that has
    // expired lands here signed out. A dead "Not logged in." left no way
    // forward; the sign-in link is the same one the role chooser uses.
    return (
      <div className="mx-auto max-w-lg space-y-3 px-4 py-6">
        ...
        <button
          type="button"
          onClick={() =>
            navigate(location.pathname === '/vendor' ? '/vendor?role=vendor&auth=login' : '/pharmacy?role=pharmacy&auth=login')
          }
